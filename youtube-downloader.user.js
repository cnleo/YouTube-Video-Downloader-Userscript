...
(function() {
	var FORMAT_LABEL = {
		...
		'171': 'WebM - audio - vorbis (128kbps)',
		'249': 'WebM - audio - opus (LQ)',
		'250': 'WebM - audio - opus (MQ)',
		'251': 'WebM - audio - opus (HQ)',
	...
	// ???FORMAT_ORDER only for FORMAT_RULE max values???
	var FORMAT_ORDER = ['251','171','18', '43', '135', '44', '22', '298', '45', '137', '299', '46', '264', '138', '266', '140'];
	var FORMAT_RULE = {
		'mp4': 'all',
		'webm': 'all',
		'm4a': 'all',
	...
	function run() {
		...
		for (var category in FORMAT_RULE) {
			if (FORMAT_RULE.hasOwnProperty(category)) {
				...
				crossXmlHttpRequest({
					...
					onload: function(response) {
						if (response.readyState === 4 && response.status === 200 && response.responseText) {
							...
							for (var i = 0; i < formats.length; i++) {
								var k = formats[i];
								if (videoURL[k] || showFormat[k] == false) {
									continue;
								}
		...
		function fetchSignatureScript(scriptURL) {
			...
			if (
				storageCode &&
				isValidSignatureCode(storageCode) &&
				storageURL &&
